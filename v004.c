...
//float round(float);//rount number to 4 digits after koma;

char userinput[MAXLINE] = {0};//user input;
char allowed_chars[] = {'^', '(', ')', '/', '*', '+', '-', '.', '\0'};//allowed operators
...
void form(char* pn) {
	...
	for (int i = 0; pn[i] != '\0' && i < MAXLINE && j < MAXLINE; i++) {
		if (digit(pn[i]) >= 0) { //incarr(pn[i], allowed_chars)) {
			clean_arr[j++] = pn[i];
...
void colektarguments(char* arr1, float* arr2) {
	...
	printf("c = %d;\n", c);
	for (int i = arrlen(arr1) - 1; i >= 0; i--) {
		//printf("Aktual char: %c\n", arr1[i]);
		int d = digit(arr1[i]);
		if (d > -1) {
			arr2[c] = arr2[c] + d * rs;
			//printf("New element = %c\n", arr1[i]);
			rs = rs * 10;
		} else if (incarr(arr1[i], allowed_chars)) {
			//printf("New char: %c\n", arr1[i]);
			arr2[c] = ((float) arr1[i])/1000000;
		} else if (arr1[i] == ' ') {
			rs = 1.0;
			c--;
			printf("c = %d;\n", c);
		}
	} printf("cycel is done\n");
}
