...
/*float openprentches(int len, float *exp, int start, int end)
{
	...
	{
		//printf("exp: %f, i: %d\n", exp[i], i);
		if (exp[i] == operators[8] || exp[i] == operators[9])
		{
			if (et < 0)
			{
				return operators[7];
	...
	{
		printf("	<<<=== Prantches found ===>>>\n");
		int st, ed, io;
		st = ed = io = 0;
		for (int e = start; e < end; e++)
		{
			if (exp[e] == operators[8])
			{
				if (io == 0)
				{
					st = e;
			...
			else if (exp[e] == operators[9])
			{
				io--;
				if (io == 0)
				{
					ed = e;
					exp[ed] = exp[st] = operators[6];
					printf("End: %d\n", ed);
					openprentches(len, exp, st+1, ed-1);
					e = start;
					st = ed = io = 0;
				}
			}
