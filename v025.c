	...
	int ap = 0;
		...
		for (int f = 0; args[h][f] != 0; f++)
		{
			if (args[h][f] != ' ' && args[h][f] != '\'' && args[h][f] != '\"')
			{
				...
				if (!((-1 != digit(userinput[ap])) || (userinput[ap] == '\'') || (userinput[ap] == '\"') || (incarr(userinput[ap], allowed_chars))))
				{
					printf("Error\n");
					return 1;
				}
		...
		if (incarr(arr1[i], allowed_chars) || arr1[i+1] == '\0' || arr1[i+1] == '\n')
		{
			...
			else
			{
				if (afp > 0)
				{
					...
					arr2[c-1] = arr2[c] = operators[6];//skip;
					//printf("N: %f\n", arr2[c-2]);
				}
				afp = 0;
			}
...
float calculate(int len, float* exp, int start, int end)
{
	...
	for (int i = start; i < end; i++)
	{
		int op1, op2;
		int counter;
		for (counter = i-1; (counter > start) && (exp[counter] == operators[6]); counter--);
		op1 = counter;
		for (counter = i+1; (counter < end) && (exp[counter] == operators[6]); counter++);
		op2 = counter;
		//printf("Op1: %d, Op2: %d, start: %d, end: %d\n", op1, op2, start, end);
		if (exp[i] != operators[6])
		{
			//printf("EXP I: %f OP1: %f OP2: %f", exp[i], exp[op1], exp[op2]);
			if (p == 0 && exp[i] == operators[1])
			{
				exp[i] = rd((float) pow((double) exp[op1], (double) exp[op2]));
				exp[op1] = exp[op2] = operators[6];
			}
...
float openprentches(int len, float *exp, int start, int end)
{
	...
	while (cont)
	{
		...
		printf("\n");
		scanf("%d", &lol);*/
		if (infarr(len, operators[7], exp, 0, len)) { return operators[7]; }
		float result;
		...
		else
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				else if (exp[i] ==  operators[9])
				{
					...
					if (io < 0)
					{
						//printf("On prentches is missing ERROR;\n");
						exp[i] = operators[7];
						return  operators[7];
					}
					if (io == 0)
				...
				else if (!(i+1 < end) && st >= 0 && ed <= 0)
				{
					//printf("I: %d, End: %d, st: %d, ed: %d\n", i, end, st, ed);
					//printf("Off prentches is missing ERROR;\n");
					exp[i] = operators[7];
					return  operators[7];	
				}
			}
