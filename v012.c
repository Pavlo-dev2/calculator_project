...
float calculate(int len, float* exp);//calculate the result;
...
int main()
{
	...
	/*float result = */calculate(size, expression);
	//printf("Result: %f\n", result);
	return 0;
...
float calculate(int len, float* exp)
{
	printf("Calculate is runing;\n");
	char p = 0;
	for (int i = 0; i < len; i++)
	{
		printf("Calculate cycel is runing;\n");
		if (exp[i] == (float) SKIP) {continue;}
		printf("Don't skiped...\n");
		printf("%f\n", exp[i]);
		if (p == 0 && exp[i] == (float) '^' / 1000000)
		{
			printf("Exeuting...\n");
			exp[i] = rd((float) pow((double) exp[i-1], (double) exp[i+1]));
			exp[i-1] = exp[i+1] = (float) SKIP;
			printf("Result_test = %f\n", exp[i]);
